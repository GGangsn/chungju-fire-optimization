  ...
  <script>
    ...
    function onMapClick(e) {{
      ...
      geojsonLayer.eachLayer((layer) => {{
        ...
        const isTarget = turf.booleanPointInPolygon(clickPoint, layer.feature);
        
        // 2. 거리 계산
        let distance = e.latlng.distanceTo(centerLatLng);
        
        // 3. 본진 구역인 경우 거리를 0으로 처리하여 무조건 최고의 안전 혜택을 받도록 함
        if (isTarget) {{
          distance = 0;
        }}
        ...
        // 본진 구역이거나, 2km 이내 대응 구역인 경우에만 켜짐
        if (isTarget || distance <= searchRadiusMeters) {{
          // 반경 내 지역: 안전 완화된 색상 적용 + 테두리 굵게 강조
          layer.setStyle({{
            fillColor: newColor,
            fillOpacity: 0.40,  // 원래(0.22)보다 약간 더 선명하게 강조
            color: newColor,
            weight: 3.0,       // 테두리 뚜렷하게
            opacity: 0.85
          }});
          ...
          // 반경 외 지역: 색상을 끄지 않고 원래/완화된 위험도 색깔을 그대로 유지 (투명도 정상 복구)
          layer.setStyle({{
            fillColor: newColor,
            fillOpacity: 0.22,
            color: newColor,
